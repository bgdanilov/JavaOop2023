...
/* Описание класса, заметки:
    ...
    Vector vector7 = Vector.getSum(vector3, vector5);
    --------------------------------
    - здесь функция возвращает ссылку на вектор-результат, сам объект создается внутри функции,
    т.е. new Vector(...) для создания объекта выполняется, но выполняется внутри функции.
    Здесь в переменную vectorH записывается ссылка на объект, созданный внутри функции.

    Vector vector6 = new Vector(Vector.getSum(vector3, vector5));
    --------------------------------
    - здесь ссылка на вектор-результат, полученная из функции,
    передается в конструктор копирования Vector(Vector vector),
    который создает новый объект.
    Поэтому здесь в переменную vectorG записывается ссылка не на сам вектор-результат,
    а на его копию, созданную конструктором копирования.
    При этом создается 2 равных объекта, в чем нет необходимости
 */